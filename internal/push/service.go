...
// NotifyShotFinished fires when a live shot has been persisted. Runs in
// its own goroutine — callers need not block.
...
func (s *Service) NotifyAnalysisReady(shotID, model string) {
	...
	body := "AI critique ready — tap to read."
	if model != "" {
		body = model + " critique ready."
	}
	go s.broadcast(Payload{
		Title: "Analysis ready",
		Body:  body,
		Tag:   "caffeine-analysis-" + shotID,
		URL:   "/history/" + shotID,
		Kind:  "analysis_ready",
	}, selectAnalysisReady)
}
	...
	for _, sub := range subs {
		if sub.Endpoint == endpoint {
			return s.sendOne(ctx, sub, Payload{
				Title: "Caffeine test notification",
				Body:  "Push is working. Pull a shot to see the real thing.",
				Tag:   "caffeine-test",
				URL:   "/",
				Kind:  "test",
			})
...
func selectShotFinished(s Subscription) bool  { return s.OnShotFinished }
func selectAnalysisReady(s Subscription) bool { return s.OnAnalysisReady }
