...
// for self-hosted Caffeine installs. It persists browser push subscriptions
// in the same SQLite database the rest of the app uses and fans out
// notifications on "shot finished" and "analysis ready" events.
package push
...
CREATE TABLE IF NOT EXISTS push_subscriptions (
    ...
    auth              TEXT NOT NULL,
    on_shot_finished  INTEGER NOT NULL DEFAULT 1,
    on_analysis_ready INTEGER NOT NULL DEFAULT 1,
    user_agent        TEXT,
...
func (s *Store) Upsert(ctx context.Context, sub Subscription) error {
	...
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO push_subscriptions (
			...
			on_shot_finished, on_analysis_ready,
			user_agent, created_at, updated_at
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(endpoint) DO UPDATE SET
			p256dh            = excluded.p256dh,
			auth              = excluded.auth,
			on_shot_finished  = excluded.on_shot_finished,
			on_analysis_ready = excluded.on_analysis_ready,
			user_agent        = excluded.user_agent,
			updated_at        = excluded.updated_at`,
		sub.Endpoint, sub.P256dh, sub.Auth,
		boolInt(sub.OnShotFinished), boolInt(sub.OnAnalysisReady),
...
func (s *Store) List(ctx context.Context) ([]Subscription, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT endpoint, p256dh, auth, on_shot_finished, on_analysis_ready,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.Endpoint, &s.P256dh, &s.Auth,
			&shot, &analysis, &s.UserAgent, &created, &updated); err != nil {
			return nil, err
		}
		s.OnShotFinished = shot != 0
		s.OnAnalysisReady = analysis != 0
