...
func handlePushSubscribe(store *push.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		prefs := push.Preferences{OnShotFinished: true, OnAnalysisReady: true}
		...
		sub := push.Subscription{
			...
			Auth:            body.Keys.Auth,
			OnShotFinished:  prefs.OnShotFinished,
			OnAnalysisReady: prefs.OnAnalysisReady,
		...
		slog.Info("push: subscribed",
			...
			"on_shot_finished", prefs.OnShotFinished,
			"on_analysis_ready", prefs.OnAnalysisReady,
		)
		writeJSON(w, http.StatusOK, map[string]any{
			"ok":          true,
			"preferences": prefs,
